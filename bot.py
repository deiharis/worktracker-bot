...
async def mystats(interaction: discord.Interaction):
    ...
    readable_total = format_duration(total_seconds)
    recent = sessions[-5:]
    ...
    embed.add_field(
        ...
        value=f"{readable_total} across {len(sessions)} session(s)",
        inline=False
    )
...
async def editsessions(interaction: discord.Interaction):
    ...
    if not sessions:
        await interaction.response.send_message(
            "You have no completed sessions to edit.",
            ephemeral=True
        )
        return
...
@app_commands.describe(
    session_number="The session number shown by /editsessions (e.g. 3)",
...
async def edittime(
    ...
    try:
        ...
        await interaction.response.send_message(
            "❌ Invalid time format. Please use `YYYY-MM-DD HH:MM` — for example: `2024-06-01 09:30`",
            ephemeral=True
        )
        return

    target_user_session = user_sessions[session_number - 1]

    global_index = None
    for i, s in enumerate(data["completed_sessions"]):
        if (s["user_id"] == user_id
                and s["clock_in"] == target_user_session["clock_in"]
                and s["clock_out"] == target_user_session["clock_out"]):
            global_index = i
            break

    if global_index is None:
        await interaction.response.send_message(
            "❌ Could not locate that session in the log. Please try again.",
            ephemeral=True
        )
        return
    ...
    if field == "clock_in":
        if new_dt >= old_clock_out:
            await interaction.response.send_message(
                f"❌ The new clock-in time (`{new_time} UTC`) must be **before** the clock-out time "
        ...
        new_clock_in = new_dt
        new_clock_out = old_clock_out
    else:
        if new_dt <= old_clock_in:
            await interaction.response.send_message(
                f"❌ The new clock-out time (`{new_time} UTC`) must be **after** the clock-in time "
        ...
        new_clock_in = old_clock_in
        new_clock_out = new_dt
    ...
    embed.add_field(
        name="Recalculated Duration",
        value=format_duration(new_total_seconds),
        inline=False
    )
